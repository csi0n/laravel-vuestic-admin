<template>
    <div class="form-elements">
        <div class="row">
            <div class="col-md-12">
                <vuestic-widget headerText="编辑权限">
                    <form>
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset>
                                    ...
                                    <div class="form-group">
                                        <button class="btn btn-info" @click="patchPermission">
                                            保存
                                        </button>
                                    </div>
                                </fieldset>
...
<script>
    import {patchSystemPermission, editSystemPermission} from './../../../api/api';
    ...
    export default {
        ...
        methods: {
            fetchPermission() {
                editSystemPermission(this.form).then((response) => {
                    this.form = response.data.data;
                })
            },
            patchPermission() {
                patchSystemPermission(this.form).then((response) => {
                    toastr.success("成功", response.data);
                    this.$router.push({name:'system.permission.lists'});
                });
            }
        }
    }
